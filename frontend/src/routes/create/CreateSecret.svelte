<script lang="ts">
  ...
  async function createSecret(e: Event) {
    e.preventDefault();
    if (!secret.trim()) {
      error = t('create.form.errorEmpty');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('create.form.errorCreate'));
      }
      ...
      error = err instanceof Error ? err.message : t('create.form.errorGeneric');
    } finally {
      loading = false;
    }
  }
      ...
      <form onsubmit={createSecret} class="space-y-5">
        ...
        <div class="grid md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-gray-300"
            >
              <option value={1}>{t('create.form.expires.1h')}</option>
              <option value={6}>{t('create.form.expires.6h')}</option>
              <option value={24}>{t('create.form.expires.24h')}</option>
              <option value={72}>{t('create.form.expires.3d')}</option>
              <option value={168}>{t('create.form.expires.7d')}</option>
            </select>
          </div>
        </div>
        ...
        <div class="border-t border-gray-200 pt-5">
          <label class="flex items-center">
            ...
            />
            <span class="ml-2 text-sm text-gray-700">{t('create.form.extendableLabel')}</span>
          </label>
          <p class="text-xs text-gray-500 mt-1 ml-6">
            {t('create.form.extendableHint')}
          </p>
        </div>
        ...
        >
          {loading ? t('create.form.submitting') : t('create.form.submitButton')}
        </button>
      </form>
    </div>
  {:else}
      ...
      <div class="space-y-6">
        <div class="bg-yellow-50 border border-yellow-300 rounded-lg p-5">
          <div class="block text-sm font-semibold text-gray-900 mb-3">
            {t('create.result.passphraseLabel')}
          </div>
          <div class="flex items-center gap-2">
            ...
            >
              {copySuccess === 'passphrase' ? t('create.result.copied') : t('create.result.copyButton')}
            </button>
          </div>
          <p class="text-sm text-yellow-800 mt-3">
            {t('create.result.passphraseHint')}
          </p>
        </div>

        <div class="bg-blue-50 border border-blue-300 rounded-lg p-5">
          <div class="block text-sm font-semibold text-gray-900 mb-3">
            {t('create.result.shareLinkLabel')}
          </div>
          <div class="flex items-center gap-2">
            <input
              type="text"
            ...
            <button
              ...
              class="px-4 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors font-medium whitespace-nowrap"
            >
              {copySuccess === 'url' ? t('create.result.copied') : t('create.result.copyButton')}
            </button>
          </div>
        </div>
        ...
        <div class="bg-gray-50 border border-gray-200 rounded-lg p-5">
          <p class="text-sm text-gray-600">
            <strong>{t('create.result.expires')}</strong> {new Date(result.expires_at).toLocaleString(getDateLocale())}
          </p>
        </div>
      </div>
